...
def main(args):
    """Filter-in NOREGION-ing segmentations and plot a random segmented read to file."""
    # load dataframes:
    ...
    if (args.read is None):
        row_values  = choice(polya[polya['qc_tag'] == 'NOREGION'][['readname', *state_starts]].values).tolist()
        ...
        try:
            ...
            row_values = choice(read_df[read_df['qc_tag'] == 'NOREGION'][['readname', *state_starts]].values).tolist()
            read_id = row_values.pop(0)
            state_start_indices = OrderedDict()
            for k in range(len(state_starts)):
                state_start_indices[state_starts[k]] = row_values[k]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Plot a random noregion (passing in the original) segmentation from a polya output file.")
