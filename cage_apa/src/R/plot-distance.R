...
option_list <- list(
  make_option(
    c("-i", "--ifile"),
    type = "character",
    help = "Input table. For stdin use \"stdin\"", metavar = "File"
  ),
  make_option(
    c("-d", "--distance"),
    type = "integer", default = 10,
    help = "Maximum distance of the \"closest\" to consider. Default: 10."
  ),
  make_option(
    c("-s", "--direction"),
    type = "character",
  ...
  make_option(
    c("-o", "--ofile"),
    type = "character",
    help = "Output figure [pdf|ps]", metavar = "File"
  )
)
...
if (opt$collapse == TRUE) {
  ...
  print("Keeping all positions (not collapsing).")
}
...
if (output_type == "pdf") {
  ...
  if (length(xlims_plot) <= 100) {
    ...
    scale_x <- c(
      min(xlims_plot), round(min(xlims_plot) / 4, 0) * 3, round(min(xlims_plot) / 2, 0), round(min(xlims_plot) / 4, 0) * 1,
      0,
